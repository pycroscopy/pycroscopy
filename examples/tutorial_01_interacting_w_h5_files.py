...
processing are written to the same h5 file that contains the recorded measurements. The Heirarchical data format (HDF5)
allows data to be stored in multiple datasets in a tree-like manner. However, certain rules and considerations have
been made in pycroscopy to ensure consistent and easy access to any data. pycroscopy.hdf_utils contians a lot of
utility functions that simplify access to data and this tutorial provides an overview of many of the these funcitons
"""
...
# quickly visualzize all the datasets and datagroups within the file within python.
...
# * Other datasets
# For more information, please refer to the pycroscopy data format.
#
...
#
# hdf_utils has a few handy functions that simply many of these usecases:
...
# The BEPS dataset used in this example has a spectra for each combination of three other paramaters (DC offset,
# Field, and Cycle). Thus, this dataset has three new spectral dimensions in addition to the spectra itself. Hence,
# this dataet becomes a 2+4 = 6 dimensional dataset
#
...
# set of anaysis and processing functions to be reused for multiple image formats or modalities.
#
...
# tedious very quickly. pycroscopy's PycroDataset class makes such necessary informaiton and any necessary
...
# makes the spectral and positional dimensions, sizes immidiately apparent among other things
...
################################################################################################
# The functionalities of PycroDataset are enabled through several functions in hdf_utils. Below, we provide
...
# this functionality can be accesed in PycroDatasets via:
...
# In other words, the dimensions in these ancillary matricies are not arranged from fastest-varying to slowest.
# To account for such discrepencies, hdf_utils has a very handy function that goes through each of the columns or
# rows in the ancillary indices matricies and finds the order in which these dimensions vary.
#
...
if success:
    print('Succeeded in reshapring flattened 2D dataset to N dimensions')
