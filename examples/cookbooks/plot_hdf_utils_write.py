...
# Note that assign_group_index() does not create the group; it only assigns a non-conflicting string name for the group.
...
# dataset to another. To illustrate, let us copy the attributes from h5_meas_group to h5_some_dataset:
...
# datasets do not need to be Pycroscopy Main datasets, we could simply use the basic functionality of h5py to create the
# dataset
...
########################################################################################################################
# The datased named 'Other' has a new attribute named 'Mysterious_Dataset'. Let us show that this dataset is none other
# than Raw_Data:
...
# where x is the index of the group. The create_results_group() function makes it very easy to create a group with such
...
########################################################################################################################
# Looking at the contents of Normalized_Data-Fitting_000 now reveals that we have added the Position datasets as well.
# However, we still do not have the main dataset.
...
# to create new Spectroscopic_Indices and Spectroscopic_Values datasets in the same folder to express the 0 dimensions
# in the spectroscopic axis for this new dataset. However, we already have datasets of this name that we created above
# using the write_reduced_spec_dsets() function. Recall, that the criterion for a Main dataset is that it should have
# attributes of name 'Spectroscopic_Indices' and 'Spectroscopic_Values'. It does not matter what the actual name of the
# linked datasets are. Coming back to the current example, we could simply ask write_main_dataset() to name the
# spectroscopic datasets with a different prefix - 'Empty_Spec' instead of 'Spectroscopic' via the 'aux_spec_prefix'
...
# make sure that it is indeed possible to write the new data to the file. is_editable_h5() is a handy function for this
# very purpose:
