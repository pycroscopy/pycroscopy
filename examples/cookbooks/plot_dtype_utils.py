...
# validate_dtype()
# -----------------
# The validate_dtype() function ensure that a provided object is indeed a valid h5py or numpy data type. When writing a
# main dataset along with all ancillary datasets, pycroscopy meticulously ensures that all intputs are valid before
...
# --------------------------
# One common hassle when dealing with compund / structured array dtypes is that it can be a little challenging to
...
# real-values. Thus, applying this function to a complex valued dataset of size (a, b, c) would result in a real-valued
# dataset of shape (a, b, 2 * c):
...
# ---------------------------
# This function flattens a compound-valued dataset of shape (a, b, c) into a real-valued dataset of shape (a, b, k * c)
#  where k is the number of fields within the structured array / compound dtype. Here we will demonstrate this on a 1D
# array of 5 elements each containing 'r', 'g', 'b' fields:
...
# the need to explicitely call the aforementioend functions (that still do the work). Here is an example of the function
...
################################################################################
# The next three functions perform the inverse operation of taking real-valued matricies or datasets and converting them
...
# -----------------------------
# This function performs the inverse of flatten_to_real() - stacks the provided real-valued dataet into a complex or
