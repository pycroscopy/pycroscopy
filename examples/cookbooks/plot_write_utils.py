...
# the Bias dimension in that they have linearly increasing values.
...
# make_indices_matrix()
# ---------------------
# One half of the work for generating the ancillary datasets is generating the indices matrix. The make_indices_matrix()
#  function solves this problem. All one needs to do is supply a list with the lengths of each dimension.  The result is
#  a 2D matrix of shape: (3 dimensions, points in Bias[16] * points in Field[2] * points in Cycle[3]) = (3, 96).
...
# -------------------------
# make_indices_matrix() is a very handy function but it only sovles one of the problems - the indices matrix. We also
...
# the indices vector is a saw-tooth waveform that also repeats in the same manner.  The repeated + tiled indices and
# values vectors for Cycle and Field look the same / very similar since they were simple linearly increasing values to
# start with.
...
# In such cases, it helps in chunking the dataset manually. The calc_chunks() function helps in calculating appropriate
# chunk sizes for the dataset using some apriori knowledge about the way the data would be accessed, the size of each
# element in the dataset, maximum size for a single chunk, etc. The examples below illustrate a few ways on how to use
# this function:
...
# clean_string_att()
# -------------------
# As mentioned in our HDF5 primer, the h5py package used for reading and manipulating HDF5 files has issues which
# necessitate the encoding of attributes whose values are lists of strings. The clean_string_att() encodes lists of
