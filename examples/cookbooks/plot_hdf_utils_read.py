...
# Load the dataset
# ----------------
# In order to demonstrate the many functions in hdf_utils, we will be using an pycroscopy HDF5 data file generated from
# an atomic force microscope containing real experimental data and some analysis results. First, let us download this
# file from the pycroscopy Github project:
...
# print_tree() to quickly visualize all the datasets and datagroups within the file within python.

print('Contents of the H5 file:')
px.hdf_utils.print_tree(h5_f)

########################################################################################################################
# By default, print_tree() presents a clean tree view of the contents of the group. Alternatively, it can print the full
# paths of each dataset and datagroup by setting the 'full_paths' keyword argument. print_tree() could also be used to
# display the contents of and HDF5 group instead of complete HDF5 file as we have done above. Lets configure it to print
# the relative paths of all objects within the Channel_000 group:
...
########################################################################################################################
# get_attributes() is also great for only getting selected attributes. For example, if we only cared about the user and
# project related attributes, we could manually request for any that we wanted:
...
# datasets, other datasets, Groups etc.). Lets apply this function to see which of the objects within the 'Channel_000'
# Group are Main datasets:
...
# (HDF5 groups whose name starts with "Raw_Data-SHO_Fit" in this case). Before we demostrate check_for_old(), lets take
# a look at the attributes stored in the existing results groups:
...
# another. This is precisely the capability that is leveraged to turn Central datasets into Pycroscopy Datasets or
...
# in the spectral and position ancillary datasets. reshape_to_n_dims() is a very useful function that can help retrieve
# the N-dimensional form of the data using a simple function call:
