...
class PtychographyTranslator(Translator):
    ...
    def translate(self, h5_path, image_path):
        ...
        print np.max(mean_ronch), np.min(mean_ronch)
    ...
    def __getfilelist(self, path, ftype='all'):
        ...
        ftype : (optional) string / unicode 
            file type to return in file_list, Default is 'all'   
        
        Returns
        ----------
        file_list : list of strings
            names of all files in directory located at path
        numfiles : unsigned integer
        ...
        for this_file in file_list:
            split = os.path.splitext(this_file)
            if len(split) <2:
                continue
            ext = split[1]
            if ext == ftype:
                new_file_list.append(this_file)
    ...
    def __setupH5(self, num_files, hdf, usize, vsize, data_type, num_pixels, scan_size):
        ...
        main_parms = {'num_images': num_files,
                      ...
                      'scan_size_x': scan_size,
                      'scan_size_y': scan_size}
    # Create the hdf5 data Group
        root_grp = MicroDataGroup('/')
        root_grp.attrs = root_parms
        root_grp.attrs['data_type'] = 'PtychographyData'
    ...
    # Add datasets as children of Measurement_000 data group
        ...
        print('Writing following tree to thisfile:')
        root_grp.showTree()
