...
class BESHOmodel(Model):
    ...
    def __init__(self, h5_main, variables=['Frequency']):
        ...
        # self.map = mp.Poolpool = mp.Pool(processors)
    ...
    def _guessMethodsInit(self, **kwarg):
        gm = GuessMethods()
    ...
    def computeGuess(self, data, strategy='wavelet_peaks', options={"peak_widths": np.array([10,200])}, **kwargs):
        '''
        ...
        if self._parallel:
            ...
            # self._getDataChunk()
            # outside call to initialize guess method function
            ...
            while self.data is not None:  # as long as we have not reached the end of this data set:
                ...
                tasks = [(vector,self) for vector in data]
                # self.tasks = tasks
                chunk = int(self.data.shape[0] / processors)
                jobs = pool.imap(guess_methods_init, tasks, chunksize=chunk)
                # jobs = self.imap(wrapper_func, tasks)
        ...
        #             chunk = int(self.data.shape[0] / processors)
        #             jobs = pool.imap(unwrap_self_f, tasks, chunksize=chunk)
        #             # jobs = self.imap(wrapper_func, tasks)
        #
        #             # get Results from different processes
        #             print('Extracting Guesses...')
